// Esqueleto de um simulador de memÃƒÂ³ria virtual.
// Feito para a disciplina DCC065 - Sistemas Operacionais (UFMG)
// Baseado no cÃƒÂ³digo python:
...
// Campos da tabela de pÃƒÂ¡ginas
#define PT_FIELDS 7           // 4 campos na tabela
#define PT_FRAMEID 0          // EndereÃƒÂ§o da memÃƒÂ³ria fÃƒÂ­sica
#define PT_MAPPED 1           // EndereÃƒÂ§o presente na tabela
#define PT_DIRTY 2            // PÃƒÂ¡gina dirty
...
#define PT_REFERENCE_COUNTER 6// Contador para mfu
...
// Define a funÃƒÂ§ÃƒÂ£o que simula o algoritmo da polÃƒÂ­tica de subst.
...
// Codifique as reposiÃƒÂ§ÃƒÂµes a partir daqui!
// Cada mÃƒÂ©todo abaixo retorna uma pÃƒÂ¡gina para ser trocada. Note tambÃƒÂ©m
// que cada algoritmo recebe:
// - A tabela de pÃƒÂ¡ginas
// - O tamanho da mesma
// - A ÃƒÂºltima pÃƒÂ¡gina acessada
// - A primeira modura acessada (para fifo)
// - O nÃƒÂºmero de molduras
// - Se a ÃƒÂºltima instruÃƒÂ§ÃƒÂ£o gerou um ciclo de clock
//
// Adicione mais parÃƒÂ¢metros caso ache necessÃƒÂ¡rio
	...
	for(i=0; i<num_pages; i++){
		if((page_table[i][PT_FRAMEID] < page || page_table[i][PT_FRAMEID] == page_table[prev_page][PT_FRAMEID]+1 ) && page_table[i][PT_MAPPED] != 0 ){
			if(page_table[i][PT_REFERENCE_BIT]==1){
				...
				page_aux = page_table[i];
				for(j=i;j<num_pages;j++){
					page_table[j] = page_table[j+1];
				}
				page_table[num_pages-1]=page_aux;
				i-=1;
			}else{
				page = i;
			}
...
int nru(int8_t** page_table, int num_pages, int prev_page,
    ...
    while (page_table[page][PT_MAPPED] == 0 && page_table[page][PT_REFERENCE_BIT] == 0) // Encontra pÃ¡gina mapeada
        page = rand() % num_pages;
    return page;
}
...
int aging(int8_t** page_table, int num_pages, int prev_page,
    ...
    int page = prev_page;
	for(i=0; i < num_pages; i++){
		if((page_table[i][PT_AGING_COUNTER] < page_table[page][PT_AGING_COUNTER]) && (page_table[i][PT_MAPPED] != 0 ) ){
			page = i;
		}
...
int mfu(int8_t** page_table, int num_pages, int prev_page,
    ...
    int page = prev_page;
	for(i=0; i < num_pages; i++){
		if((page_table[i][PT_REFERENCE_COUNTER] > page_table[page][PT_REFERENCE_COUNTER]) && (page_table[i][PT_MAPPED] != 0 ) ){
			page = i;
		}
		
	}
	page_table[page][PT_REFERENCE_COUNTER]++;
    return page;
...
int random_page(int8_t** page_table, int num_pages, int prev_page,
                int fifo_frm, int num_frames, int clock) {
    int page = rand() % num_pages;
    while (page_table[page][PT_MAPPED] == 0) // Encontra pÃƒÂ¡gina mapeada
        page = rand() % num_pages;
    return page;
}
...
int find_next_frame(int *physical_memory, int *num_free_frames,
    ...
    // Procura por um frame livre de forma circula na memÃƒÂ³ria.
    // NÃƒÂ£o ÃƒÂ© muito eficiente, mas fazer um hash em C seria mais custoso.
    do {
...
int simulate(int8_t **page_table, int num_pages, int *prev_page, int *fifo_frm,
    ...
    int next_frame_addr;
    if ((*num_free_frames) > 0) { // Ainda temos memÃƒÂ³ria fÃƒÂ­sica livre!
    ...
    } else { // Precisamos liberar a memÃƒÂ³ria!
        ...
        *fifo_frm = (*fifo_frm + 1) % num_frames;
        ...
        page_table[to_free][PT_REFERENCE_COUNTER] = 0;
        
    }

    // Coloca endereÃƒÂ§o fÃƒÂ­sico na tabela de pÃƒÂ¡ginas!
...
int main(int argc, char **argv) {
    ...
    // Aponta para cada funÃƒÂ§ÃƒÂ£o que realmente roda a polÃƒÂ­tica de parse
    ...
    for (int i = 0; i < num_pages; i++) {
        ...
        page_table[i][PT_REFERENCE_COUNTER] = 0;
    }

    // MemÃƒÂ³ria Real ÃƒÂ© apenas uma tabela de bits (na verdade uso ints) indicando
    // quais frames/molduras estÃƒÂ£o livre. 0 == livre!
